...
{ 
    ...
    { 
        if ( gImeUIData.hListCand[ i ] ) CurNumCandList++; 
    } 
    ...
    { 
        ...
        {    
            ...
            if( !( gImeUIData.hListCandMem[ dwIndex ]  =  GlobalAlloc( LPTR, (int)dwBufLen ) ) ) 
                goto exit2; 
			
            if( !( lpStr = (LPSTR)GlobalLock( gImeUIData.hListCandMem[ dwIndex ] ) ) ) 
            {    
...
{ 
    ...
    for ( dwIndex = 0; dwIndex < MAX_LISTCAND; dwIndex++ ) 
	{
        if ( CandList & ( 1 << dwIndex ) ) break;
	}
// If dwIndex == MAX_LISTCAND, then something wrong, do nothing. 
	if ( dwIndex == MAX_LISTCAND ) return; 
...
// Determine OPAQUE rect. 
    ...
    { 
        ...
        ExtTextOut(hdc,xPos * cxMetrics,yPos * cyMetrics,ETO_OPAQUE,&rect,lpStr,StrLen,0); 
    ...
    { 
        ...
        ExtTextOut( hdc, xPos * cxMetrics, yPos * cyMetrics, ETO_OPAQUE,&rect, NULL, 0, 0 ); 
        ...
        { 
            ...
            TextOut( hdc, ( i + xPos ) * cxMetrics, yPos * cyMetrics, lpStr, cnt ); 
            lpStr += cnt; 
            lpStrAttr += cnt; 
            i += cnt; 
        } 
    ...
    if ( fDBCSTrailByte ) 
    { 
        TextOut( hdc, ( xPos + StrLen ) * cxMetrics, yPos * cyMetrics, 	" ", 1 ); 
    } 
