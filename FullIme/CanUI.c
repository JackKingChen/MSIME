...
{ 
    ...
    dwNumPerPage = ( !lpCandList->dwPageSize ) ? DEFAULT_CAND_NUM_PER_PAGE : 
    ...
    rect.top = rect.top + ( lpCandList->dwSelection - dwStartIndex ) * cyMetrics; 
...
{ 
	...
	{
        if ( gImeUIData.hListCand[ index ] == hwnd ) break;
	}
	// If index == MAX_LISTCAND, then something wrong, do nothing. 
	if ( index == MAX_LISTCAND ) goto exit; 
	
	if ( !gImeUIData.hListCandMem[ index ] || (lpCandList = (LPCANDIDATELIST) 
