...
* Title: 
* 
*   STED.C - IME Half-aware Simple Text Editor (DBCS version) 
* 
* Purpose: 
* 
...
{ 
	if ( CompWindowMode == IDC_NEARCARET_POS || CompWindowMode == IDC_PREFER_POS ) 
	{ 
		HIMC hIMC = ImmGetContext(hWnd); 
		if ( hIMC ) ImmSetCompositionWindow(hIMC,&CompForm); 
		ImmReleaseContext( hWnd , hIMC); 
	} 
} 
//********************************************************************** 
...
// Blank out text buffer.  Return caret to home position 
		...
		{
			for ( xPos = FIRSTCOL; xPos <= LASTCOL; xPos++ ) textbuf[yPos][xPos] = ' '; 
		}
	break; 
    case IDC_ANSIFILL: 
    case IDC_DBCSFILL: 
// Fill text buffer with ANSI or DBCS pattern 
		for ( yPos = FIRSTROW; yPos <= LASTROW; yPos++ )
		{
			...
			{
				if ( cmd == IDC_ANSIFILL ) textbuf[yPos][xPos] = 'a'; 
				else
				{ 
...
//		
	...
	{ 
		...
		} while ( (xPos % TABSTOP) && (xPos < xTabMax) && (xPos != xPosPrev)); 
	} 
	break; 
...
//In the Win32-based application programming interface (API),
